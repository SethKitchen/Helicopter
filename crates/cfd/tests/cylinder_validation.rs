...
fn matches_re40_cylinder_benchmark() {
    ...
    assert!(cdf > 0.0 && cdp > 0.0, "friction and pressure drag both positive");
    assert!((cd_surf - 1.52).abs() / 1.52 < 0.15, "C_D(surface) {cd_surf} vs ≈1.48–1.66");
    assert!((cd_diss - 1.52).abs() / 1.52 < 0.30, "C_D(dissipation) {cd_diss} right order");
