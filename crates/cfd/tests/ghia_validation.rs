...
fn matches_ghia_re100_lid_driven_cavity() {
    ...
    assert!(
        rel(u_min, -0.21090) < 0.05,
        "u_min {u_min} vs Ghia -0.21090"
    );
    ...
    assert!(
        rel(v_min, -0.24533) < 0.05,
        "v_min {v_min} vs Ghia -0.24533"
    );
    ...
    assert!(
        rel(vpsi, -0.103423) < 0.04,
        "ψ_min {vpsi} vs Ghia -0.103423"
    );
}
...
fn refinement_converges_toward_ghia() {
    ...
    assert!(
        fine_err < coarse_err,
        "refining must approach Ghia: {coarse_err} → {fine_err}"
    );
}
