...
pub fn main_rotor_with_inflow(
    ...
    let (rotor, op) = (aero.rotor, aero.op);
    ...
    assemble_forces(aero, mu, rates_bar, inflow)
}
...
pub fn quasi_static_inflow(
    ...
    for _ in 0..80 {
        out = main_rotor_with_inflow(aero, vel, rates, nu);
        let c = [out.1.c_t, out.1.c_roll, out.1.c_pitch];
        let (mu, lambda) = flow_scales(aero.op, aero.rotor, vel, nu[0]);
...
pub fn inflow_rate(
    ...
    let (forces, forcing) = main_rotor_with_inflow(aero, vel, rates, nu);
    let c = [forcing.c_t, forcing.c_roll, forcing.c_pitch];
    let (mu, lambda) = flow_scales(aero.op, aero.rotor, vel, nu[0]);
    let d = inflow_derivative(nu, c, mu, lambda, aero.op.omega, lag);
    (d, forces)
}
...
pub fn march_inflow(
    ...
    for _ in 0..steps {
        let (d, f) = inflow_rate(aero, vel, rates, nu, lag);
        forces = f;
        for i in 0..3 {
            nu[i] += dt * d[i];
        }
