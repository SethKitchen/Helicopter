...
fn cfd_polar_interpolates_through_the_airfoil_trait() {
    ...
    let af = CfdAirfoil::from_polar_deg(
        &[(-8.0, -0.9, 0.13), (0.0, 0.0, 0.10), (8.0, 0.9, 0.13)],
        200.0,
    );
    let (cl, cd) = af.cl_cd(4.0_f64.to_radians(), 0.0);
    assert!(
        (cl - 0.45).abs() < 1e-6 && (cd - 0.115).abs() < 1e-6,
        "midpoint interp"
    );
...
fn viterna_completes_the_polar_to_deep_stall() {
    ...
    let attached = CfdAirfoil::from_polar_deg(
        &[(-14.0, -1.0, 0.06), (0.0, 0.0, 0.04), (14.0, 1.0, 0.06)],
        500.0,
    );
    ...
    assert!(
        (full.cd(0.0, 0.0) - 0.04).abs() < 1e-6,
        "attached drag kept at α=0"
    );
    // Deep stall reaches the flat-plate limits: drag climbs toward Cd_max, lift → 0.
    assert!(
        full.cd(45f64.to_radians(), 0.0) > full.cd(14f64.to_radians(), 0.0),
        "drag rises past stall"
    );
    assert!(
        full.cd(85f64.to_radians(), 0.0) > 1.5,
        "Cd near 90° approaches flat-plate ~2.0"
    );
    assert!(
        full.cl(89f64.to_radians(), 0.0).abs() < 0.15,
        "lift → 0 at 90°"
    );
...
fn cfd_low_re_drag_penalises_the_rotor_vs_analytic_high_re() {
    ...
    assert!(
        cd_cfd > 5.0 * cd_lin,
        "low-Re Cd {cd_cfd} ≫ high-Re Cd {cd_lin}"
    );
    ...
    assert!(
        h_cfd.ct > 0.0 && h_cfd.cp > 0.0,
        "CFD-airfoil hover is physical"
    );
    assert!(
        h_cfd.figure_of_merit > 0.0 && h_cfd.figure_of_merit < 1.0,
        "FM in range"
    );
