...
mod tests {
    ...
    fn high_thrust_grows_the_bracket_and_zero_thrust_has_no_induced_power() {
        // A large C_T at low μ forces the bracket-growth loop: initial hi=1 gives
        // momentum_ct≈2 < 8, so `hi` must double before it brackets the root.
        let solved = glauert_inflow(8.0, 0.05, 0.0, 1e-10, 200);
        assert!((momentum_ct(solved, 0.05, 0.0) - 8.0).abs() < 1e-3);
