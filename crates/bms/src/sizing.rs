...
pub fn size_for_target(cell: &dyn Cell, true_continuous_a: f64, target: Target) -> PackSizing {
    ...
    // Power: peak pack current = P_peak / V_bus; per-cell must stay ≤ I_cont.
    let p_for_power = (target.peak_power_w / (bus_nominal_v * true_continuous_a)).ceil();
    ...
    let peak_pack_current = target.peak_power_w / bus_nominal_v;
    let peak_cell_current_a = peak_pack_current / p;
    PackSizing {
        series,
        parallel,
